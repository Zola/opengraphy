...
var zhTWExtra = map[string]string{
	...
	"platform_chat_body":       "偏好伺服器初始 HTML 裡就存在的標籤；不要只靠前端 JS 動態插入 metadata。",
	"platform_visual_body":     "除了寬版圖，電商、作品集和文章頁也可準備方形或直式圖片，讓縮圖在內容流裡更醒目。",
	"framework_meta_body":      "優先使用框架提供的 metadata API 或 SSR/SSG，在伺服器輸出的 HTML 裡直接生成 OG 標籤。",
	"framework_wordpress_body": "可以使用 SEO 外掛，也可以在 theme 的 head hook 裡輸出文章標題、摘要、特色圖片和永久連結。",
	"framework_spa_title":      "單頁應用 SPA",
	"framework_spa_body":       "如果完全依靠客戶端渲染，許多平台 crawler 會看不到 metadata。請加 SSR、預渲染，或為分享頁提供靜態 HTML。",
